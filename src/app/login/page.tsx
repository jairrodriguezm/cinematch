  ...
  return (
    <section className="w-full rounded-3xl border border-neutral-200 bg-white p-6 shadow-[0_12px_40px_rgba(15,15,16,0.06)] text-black">
      ...
      <form noValidate onSubmit={handleMagicLink} className="mt-6 space-y-3">
        ...
        <button type="submit" disabled={pending !== null} className="w-full flex items-center justify-center gap-2 rounded-xl bg-[#f5c518] px-4 py-3 text-sm font-extrabold text-black transition hover:bg-amber-400 disabled:cursor-not-allowed disabled:opacity-60 shadow-md">
          {pending === 'magic' ? <><Loader2 className="h-4 w-4 animate-spin" /> Enviando enlace...</> : 'Enviar enlace mágico'}
        </button>
      </form>
      ...
      <button type="button" onClick={handleGoogle} disabled={pending !== null} className="flex w-full items-center justify-center gap-2 rounded-xl border border-neutral-200 px-4 py-3 text-sm font-semibold text-[#0F0F10] transition hover:bg-neutral-50 disabled:cursor-not-allowed disabled:opacity-60">
        {pending === 'google' ? <Loader2 className="h-5 w-5 animate-spin text-neutral-500" /> : <span aria-hidden="true" className="text-base font-bold text-[#4285F4]">G</span>}
